...
SAML_vars = {
    ...
    'federation_id':'test',
    'file_base':'/home/staffknex/PythonSAML/',
    'key_file' : 'dl_key.pem',
    'cert_file' : 'dl.crt'
    #'key_file' : 'cakey.pem',
...
# def sign2(xmlstr):
#     issuer = NameID(SAML_vars['issuer_url'])
#     from dm.saml2.signature import default_sign_context
#     default_sign_context.add_key(xmlsec.Key.load('/home/staffknex/PythonSAML/test.pem',  xmlsec.KeyDataFormatPem, None), issuer.value())

#     xmlstr.request_signature()
#     returned xmlstr.toxml()
...
def sso(*args, **kwargs):
    ...
    signed_saml_xml = sign_file(
        ...
        str("{0}{1}".format(SAML_vars['file_base'],SAML_vars['key_file'])), 
        str("{0}{1}".format(SAML_vars['file_base'],SAML_vars['cert_file'])))
    #signed_saml_xml = sign2(saml_xml)

    #signed_saml_xml = signed_saml_xml.replace("\n","")

    print 'start signed SAML xml =============================================='    
    print signed_saml_xml
    print 'end signed SAML xml ================================================'    
